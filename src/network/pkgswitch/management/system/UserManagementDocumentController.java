...
public class UserManagementDocumentController implements Initializable
{
                ...
                final TableCell<UserManagement, Void> cell = new TableCell<UserManagement, Void>()
                {
                    private final Button btnUpdate = new Button ( "Update" );
                    {
                        btnUpdate.setOnAction ( ( final ActionEvent event ) ->
                        {
                            ...
                            try
                            {
                                UserManagementServices.updateUser ( userManagement );
                                Utils.getAlert ( Alert.AlertType.INFORMATION, "User information updated!",
                                    "User information updated successfully to database!" ).showAndWait();
                            }
                            catch ( final Exception e )
                            {
                                e.printStackTrace();

                                Utils.getAlert ( Alert.AlertType.ERROR, "Error updating user information!",
                                    "Unable to update user information to database: " + e.getMessage() ).showAndWait();
                            }
                        } );
                    }

                    private final Button btnChangePassword = new Button ( "Change Password" );
                    {
                        btnChangePassword.setOnAction ( ( final ActionEvent event ) ->
                        {
                            final UserManagement userManagement = getTableView().getItems().get ( getIndex() );

                            try
                            {
                                final FXMLLoader fxmlLoader = new FXMLLoader ( getClass().getResource ( "EnterNewPasswordDocument.fxml" ) );
                                final Parent parent = fxmlLoader.load();
                                final EnterNewPasswordDocumentController dialogController = fxmlLoader.<EnterNewPasswordDocumentController>getController();
                                dialogController.setUser ( userManagement );
                            ...
                            catch ( final IOException e )
                            {
                                ...
                                Utils.getAlert ( Alert.AlertType.ERROR, "Error opening Enter New Password dialog!", "Unable to open Enter New Password dialog: " + e.getMessage() ).showAndWait();
                            }
                        } );
                    }
                    ...
                    {
                        btnBan.setOnAction ( ( final ActionEvent event ) ->
                        {
                            final UserManagement userManagement = getTableView().getItems().get ( getIndex() );

                            try
                            {
                                UserManagementServices.setUserStatus ( userManagement, "Banned" );

                                Utils.getAlert ( Alert.AlertType.INFORMATION, "User banned!",
                                    "User banned successfully in database!" ).showAndWait();
                            ...
                            catch ( final Exception e )
                            {
                                ...
                                Utils.getAlert ( Alert.AlertType.ERROR, "Error banning user!",
                                    "Unable to ban user in database: " + e.getMessage() ).showAndWait();
                            }
                        } );
                    }
                    ...
                    private final VBox pane = new VBox ( btnUpdate, btnChangePassword, btnBan, btnDelete );
                    {
    ...
    private void btnReloadFromDatabaseAction ( final ActionEvent event )
    {
        ...
        try
        {
            alUsers = UserManagementServices.readUsers ( "Approve" );
